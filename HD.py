...
def width_function(height_i: float, _: float = 0) -> float:
    bredde_OK = 1170
    bredde_UK = 1180
    bredde_midje = bredde_UK - 180 * 5

    h_tot = 290
    h_overgang = 25
    height_UK = 50
    height_OK = h_tot - 70
    height_OK_midje = height_OK - h_overgang

    if height_i <= height_UK:
        # Konstantbredde i UK
        return bredde_UK
    if height_i > height_OK:
        # Konstantbredde i OK
        return bredde_OK
    if height_OK - h_overgang >= height_i >= height_UK + h_overgang:
        # Konstantbredde i midten
        return bredde_midje
    if height_i < height_UK + h_overgang:
        # Interpolerer mellom bredde_UK og bredde_midje
        t = (height_i - height_UK) / h_overgang
        return bredde_UK + t * (bredde_midje - bredde_UK)

    # Interpolerer mellom bredde_OK og bredde_midje
    t = (height_i - height_OK_midje) / h_overgang
    return bredde_midje + t * (bredde_OK - bredde_midje)


if __name__ == "__main__":
    ### INPUT STARTER ###
    # Bjelkens lengde
    bjelkelengde: float = 8.78  # i m

    # Betongparametere
    f_ck: float = 45  # Betongkvalitet
    materialmodell = "Parabola"  # kan velge mellom Parabola og Sargin (pass på)
    strekkfast_betong_SLS: bool = (
        True  # Velg om strekkfasthet skal inkluderes for SLS-beregninger
    )
    ...
    height = 290  # Høyden må ha like mange datapunkt som momentvektoren

    # Bredde kan være et tall eller en funksjon (funksjon er nødvendig for I-tverrsnitt)
    width = width_function  # width = width_function er også et alternativ
    kompakt_lengde = 0  # Lengde fra hvert opplegg med kompakt tverrsnitt (for SDT ol)
    ...
    as_area_bot = np.array([])  # 0 stk ø16
    as_area_top = np.array([])  # 0 stk ø16
    ...
    creep_eff: float = 0.0  # 1.61 eksempelverdi
    ...
    assert antall_punkter >= np.ceil(bjelkelengde / 0.5), (
        "Bjelkesegmenter må være minst 0.5m. Endre antall punkter"
    )
    ...
    if alpha_uls > 0.617:
        print(
            f"Alpha: {alpha_uls:.3f} -> Overarmert tverrsnitt. Trykksonehøyde: {trykksonehoyde:.1f}mm målt fra OK"
        )
    else:
        print(
            f"Alpha: {alpha_uls:.3f} -> Underarmert tverrsnitt. Trykksonehøyde: {trykksonehoyde:.1f}mm målt fra OK"
        )
